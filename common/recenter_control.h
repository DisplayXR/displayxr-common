...
//
// Lives in displayxr-common so every app gets ONE copy: it was previously
// duplicated byte-for-byte in each demo's model_common/, which is exactly the
// drift hazard this repo exists to remove. It owns the pin STATE and the label
// text; it does NOT own any on-screen placement — pair it with dxr::ToastState
// (toast.h) for the confirmation chip, whose geometry comes from
// dxr::ComputeToastLayerRect().
//
