...
CI image. The Wayland window backend of `displayxr::linux_window` (`common/linux/dxr_linux_window.cpp`)
...
this XML at build time (see `common/linux/dxr_linux_window.cmake`) and writes
...
The window chrome (`common/linux/dxr_wl_chrome.cpp`) uses four more
