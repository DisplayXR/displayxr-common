...
/*!
 * A pointer event on the CONTENT surface, forwarded to DxrLinuxWindow (the
 * seat's one wl_pointer lives here, so the chrome is the only listener).
 * Presses that start an edge resize are consumed and never forwarded.
 * Coordinates are surface-local LOGICAL px.
 */
struct DxrWlPointerEvent
{
	enum class Kind
	{
		Enter,
		Leave,
		Motion,
		Button,       //!< button (evdev BTN_*), pressed
		Axis,         //!< axis (WL_POINTER_AXIS_*), value (continuous)
		AxisDiscrete, //!< axis, discrete (wheel clicks, seat v5+)
		Frame,        //!< end of one logical pointer event (seat v5+)
	};
...
public:
	...
	//! Where content-surface pointer events go (see DxrWlPointerEvent).
	typedef void (*ContentPointerFn)(void *userdata, const DxrWlPointerEvent &ev);
	void
	set_content_pointer_sink(ContentPointerFn fn, void *userdata)
	{
		m_content_fn = fn;
...
private:
	...
	// Content pointer forwarding (DxrLinuxWindow's input events).
	ContentPointerFn m_content_fn = nullptr;
	void *m_content_userdata = nullptr;
	void
	forward(const DxrWlPointerEvent &ev)
	{
		if (m_content_fn != nullptr) {
			m_content_fn(m_content_userdata, ev);
		}
	}
};
