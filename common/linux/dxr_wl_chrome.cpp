...
{
	return (m_mode == Mode::ClientSide && !m_fullscreen && !m_hidden && !m_suppressed) ? m_bar.logicalHeight()
	                                                                                  : 0;
}
...
void
DxrWlChrome::set_title(const char *title)
{
	m_bar.setTitle(title != nullptr ? title : "");
}
	...
	if (m_ptr_surface == m_content && m_content != nullptr) {
		if (pressed && button == BTN_LEFT) {
			...
			if (edge != XDG_TOPLEVEL_RESIZE_EDGE_NONE) {
				...
				return; // an edge resize, not a click into the content
			}
		}
		DxrWlPointerEvent ev;
		ev.kind = DxrWlPointerEvent::Kind::Button;
		ev.x = m_ptr_x;
		ev.y = m_ptr_y;
		ev.serial = serial;
		ev.time = time;
...
DxrWlChrome::s_ptr_enter(void *data, struct wl_pointer *p, uint32_t serial, struct wl_surface *s, wl_fixed_t x, wl_fixed_t y)
{
	...
	if (s != nullptr && s == self->m_content) {
		DxrWlPointerEvent ev;
		ev.kind = DxrWlPointerEvent::Kind::Enter;
...
DxrWlChrome::s_ptr_leave(void *data, struct wl_pointer *p, uint32_t serial, struct wl_surface *s)
{
	...
	(void)s;
	auto *self = static_cast<DxrWlChrome *>(data);
	const bool was_content = self->m_ptr_surface != nullptr && self->m_ptr_surface == self->m_content;
	self->m_ptr_surface = nullptr;
	self->m_bar.setHover(dxr_csd::Hit::Outside);
	self->m_bar.setPressed(dxr_csd::Hit::Outside);
	if (was_content) {
		DxrWlPointerEvent ev;
		ev.kind = DxrWlPointerEvent::Kind::Leave;
		ev.serial = serial;
		self->forward(ev);
	}
}
...
DxrWlChrome::s_ptr_motion(void *data, struct wl_pointer *p, uint32_t time, wl_fixed_t x, wl_fixed_t y)
{
	...
	auto *self = static_cast<DxrWlChrome *>(data);
	self->on_pointer_motion(wl_fixed_to_double(x), wl_fixed_to_double(y));
	if (self->m_ptr_surface != nullptr && self->m_ptr_surface == self->m_content) {
		DxrWlPointerEvent ev;
		ev.kind = DxrWlPointerEvent::Kind::Motion;
		ev.x = wl_fixed_to_double(x);
		ev.y = wl_fixed_to_double(y);
		ev.time = time;
		self->forward(ev);
...
DxrWlChrome::s_ptr_axis(void *data, struct wl_pointer *p, uint32_t time, uint32_t axis, wl_fixed_t value)
{
	...
	if (self->m_ptr_surface != nullptr && self->m_ptr_surface == self->m_content) {
		DxrWlPointerEvent ev;
		ev.kind = DxrWlPointerEvent::Kind::Axis;
		ev.x = self->m_ptr_x;
		ev.y = self->m_ptr_y;
		ev.time = time;
		ev.axis = axis;
		ev.value = wl_fixed_to_double(value);
		self->forward(ev);
	}
}
...
DxrWlChrome::s_ptr_frame(void *data, struct wl_pointer *p)
{
	(void)p;
	auto *self = static_cast<DxrWlChrome *>(data);
	// Forwarded unconditionally: it closes whatever the content received in
	// this frame (a leave, even though the pointer is no longer over it).
	DxrWlPointerEvent ev;
	ev.kind = DxrWlPointerEvent::Kind::Frame;
	ev.x = self->m_ptr_x;
	ev.y = self->m_ptr_y;
	self->forward(ev);
}
...
DxrWlChrome::s_ptr_axis_discrete(void *data, struct wl_pointer *p, uint32_t axis, int32_t discrete)
{
	...
	if (self->m_ptr_surface != nullptr && self->m_ptr_surface == self->m_content) {
		DxrWlPointerEvent ev;
		ev.kind = DxrWlPointerEvent::Kind::AxisDiscrete;
		ev.x = self->m_ptr_x;
		ev.y = self->m_ptr_y;
		ev.axis = axis;
		ev.discrete = discrete;
		self->forward(ev);
	}
}
