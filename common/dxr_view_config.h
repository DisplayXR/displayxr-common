 ...
 * unconditionally.
 *
 * Header-only and C-compatible on purpose (C11 + C++17 clean, no compiled
 * source, no platform dependency): Windows apps get it through
 * displayxr::common's XrSessionManager, and the macOS/Linux/Android legs —
 * which carry their own session code and cannot link the STATIC
 * displayxr_common_lib at all — take it from displayxr::rules and apply it to
 * their own variable.
 *
 * Vendored verbatim (plus the second-stage fallback below) from
 * displayxr-runtime test_apps/common/dxr_view_config.h @ c1e4fe00d;
 * DxrAliasInactiveViews() (ADR-041, runtime #1612) moved verbatim from the
 * same file @ 402b1e242 so every demo leg shares ONE implementation.
 */
...
/*
 * The type value is normally supplied by the consumer's copy of
 * XR_DXR_display_info.h (DISPLAYXR_EXTENSIONS_INCLUDE_DIR, or the pinned
 * displayxr-extensions fetch this repo falls back to) (SPEC_VERSION >=
...
/*
 * Second stage: the header WAS found but predates SPEC_VERSION 19, so it does
 * not carry the enumerator (displayxr-common is consumed by trees that pin
 * their own vendored openxr_includes/ — a stale one must not break the build).
 * Deliberately narrower than a bare #ifndef: it fires only for a pre-19
 * XR_DXR_display_info.h, so once the enumerator is registered with Khronos and
 * arrives as a real XrViewConfigurationType enumerator (no macro to detect),
 * this cannot shadow it — same reasoning as the header probe above.
 */
#if defined(DXR_VIEW_CONFIG_HAVE_DISPLAY_INFO_HEADER) &&                                                               \
    (!defined(XR_DXR_display_info_SPEC_VERSION) || XR_DXR_display_info_SPEC_VERSION < 19)
